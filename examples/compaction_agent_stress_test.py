...
logfire.configure(
    scrubbing=False, service_name="compaction_stress_test", send_to_logfire="if-token-present"
)
...
class CompactionTestRunner:
    ...
    def print_comparison(self, before: dict, after: dict):
        ...
        pref_reduction = (
            ((before["pref_count"] - after["pref_count"]) / before["pref_count"] * 100)
            if before["pref_count"] > 0
            else 0
        )
        char_reduction = (
            ((before["total_chars"] - after["total_chars"]) / before["total_chars"] * 100)
            if before["total_chars"] > 0
            else 0
        )
        ...
        print(
            f"  • Total chars: {before['total_chars']} → {after['total_chars']} ({char_reduction:+.0f}%)"
        )
...
async def scenario_2_contradictory_evolution():
    ...
    evolution = [
        ...
        (
            "middle_episode",
            "You know, spicy food isn't as bad as I thought. I can tolerate mild spice now.",
        ),
        (
            "recent_episode",
            "I've completely changed my mind - I love really spicy food now! The hotter the better!",
        ),
    ]
...
async def scenario_3_noise_filler():
    ...
    for session_id, user_msg, assistant_msg in conversations:
        await runner.diary.update_memory(
            user_id=user_id, session_id=session_id, user_msg=user_msg, assistant_msg=assistant_msg
        )
...
async def scenario_5_volume_stress():
    ...
    for i in range(10):
        messages.append(
            (f"french_{i}", "What cuisine do you like?", "French cuisine is my favorite")
        )
...
async def run_all_scenarios():
    ...
    for scenario, (before, after) in results.items():
        ...
        print(
            f"{scenario.upper()}: {before['total_chars']} → {after['total_chars']} chars ({char_reduction:+.0f}%)"
        )
