...
async def test_fallback_model_creation(monkeypatch):
    ...
    # Our default policy (decoupled from pydantic-ai internals): retry on transient
    # model/HTTP errors, output validation failures, and request timeouts.
    assert (ModelHTTPError, ValidationError, httpx.TimeoutException) == DEFAULT_FALLBACK_EXCEPTIONS

    # And it is actually wired into the FallbackModel. `_should_fallback` is pydantic-ai's
    # async fallback predicate (stable across the supported >=1.75 range).
    assert await agent.model._should_fallback(ModelHTTPError(500, "test"))
    ...
    try:
        ...
        assert await agent.model._should_fallback(e)

    assert await agent.model._should_fallback(httpx.TimeoutException("boom"))
    # Unrelated errors should not trigger fallback.
    assert not await agent.model._should_fallback(ValueError("unrelated"))


async def test_prompt_object_handled(tmp_path, capsys):
    prompt_path = tmp_path / "prompt.txt"
    prompt_path.write_text("Test {categories_doc} {current_time}")
    prompt_obj = Prompt.from_path(prompt_path, metadata="allow")
...
def test_env_model_default(monkeypatch):
    monkeypatch.setenv("OPENAI_API_KEY", "test")
    monkeypatch.setenv("EXTRACTOR_MODEL", "openai-chat:gpt-4o-mini")
